...
private:
  ...
  void Poly_2nd(pt_t in_pt, phi_t in_phi, proj_t &proj_cos, proj_t &proj_sin) const;
...
void L1MetPfProducer::CalcMetHLS(std::vector<float> pt,
  ...
  for (uint i = 0; i < pt.size(); i++) {
    ...
    Poly_2nd(hw_pt, hw_phi, poly_x, poly_y);

    hw_sumx = hw_sumx - hw_px;
    hw_sumy = hw_sumy - hw_py;

    poly_sumx = poly_sumx - poly_x;
    poly_sumy = poly_sumy - poly_y;
  }

  pt2_t hw_met = pt2_t(hw_sumx) * pt2_t(hw_sumx) + pt2_t(hw_sumy) * pt2_t(hw_sumy);
  hw_met = sqrt(int(hw_met));  // stand-in for HLS::sqrt

  pt2_t poly_met = hypot(hw_sumx.to_float(), hw_sumy.to_float());
  phi_t out_metphi = phi_t(ap_fixed<26, 11>(atan2(poly_sumy.to_float(), poly_sumx.to_float())) * ap_fixed<26, 11>(229.29936)); //Reduce 25 cycles

  phi_t hw_met_phi = 0;
  PhiFromXY(hw_sumx, hw_sumy, hw_met_phi);

  std::cout << "Original MET: " << hw_met.to_double()
            << ", Phi: " << hw_met_phi.to_double() * phiLSB_
            << "\nNew MET: " << poly_met.to_float()
            << ", Phi: " << out_metphi.to_double() * phiLSB_
            << std::endl;
...
void L1MetPfProducer::Poly_2nd(pt_t in_pt, phi_t in_phi, proj_t &proj_cos, proj_t &proj_sin) const {
    #define NPOINT 16
    #define POLY_MAP 0.004363323129985824
    ...
    phi_t phi2_edges[NPOINT] = {-720, -630, -540, -450, -360, -270, -180, -90, 0, 90, 180, 270, 360, 450, 540, 630};
    // #pragma HLS bind_storage variable=cos2_par0 type=RAM_1P impl=BRAM
    // #pragma HLS bind_storage variable=cos2_par1 type=RAM_1P impl=BRAM
    // #pragma HLS bind_storage variable=cos2_par2 type=RAM_1P impl=BRAM

    // #pragma HLS bind_storage variable=sin2_par0 type=RAM_1P impl=BRAM
    // #pragma HLS bind_storage variable=sin2_par1 type=RAM_1P impl=BRAM
    // #pragma HLS bind_storage variable=sin2_par2 type=RAM_1P impl=BRAM

    // #pragma HLS bind_storage variable=phi2_edges type=RAM_1P impl=BRAM

    int phibin = 0;
    if      (in_phi < phi2_edges[1]) phibin = 0;
    else if (in_phi < phi2_edges[2]) phibin = 1;
    else if (in_phi < phi2_edges[3]) phibin = 2;
    else if (in_phi < phi2_edges[4]) phibin = 3;
    else if (in_phi < phi2_edges[5]) phibin = 4;
    else if (in_phi < phi2_edges[6]) phibin = 5;
    else if (in_phi < phi2_edges[7]) phibin = 6;
    else if (in_phi < phi2_edges[8]) phibin = 7;
    else if (in_phi < phi2_edges[9]) phibin = 8;
    else if (in_phi < phi2_edges[10]) phibin = 9;
    else if (in_phi < phi2_edges[11]) phibin = 10;
    else if (in_phi < phi2_edges[12]) phibin = 11;
    else if (in_phi < phi2_edges[13]) phibin = 12;
    else if (in_phi < phi2_edges[14]) phibin = 13;
    else if (in_phi < phi2_edges[15]) phibin = 14;
    else if (in_phi >= phi2_edges[15]) phibin = 15;
    // std::cout << "Phi Bin: " << phibin << std::endl;
    // poly_t poly_phi = poly_t(POLY_MAP) * (in_phi - phi2_edges[phibin]);
    poly_t cos_var = cos2_par0[phibin] + cos2_par1[phibin] * (in_phi - phi2_edges[phibin]) + cos2_par2[phibin] * (in_phi - phi2_edges[phibin]) * (in_phi - phi2_edges[phibin]);
    poly_t sin_var = sin2_par0[phibin] + sin2_par1[phibin] * (in_phi - phi2_edges[phibin]) + sin2_par2[phibin] * (in_phi - phi2_edges[phibin]) * (in_phi - phi2_edges[phibin]);
    // std::cout << "Cos: " << cos_var
    //           << ", Sin: " << sin_var
    //           << std::endl;
    proj_cos = in_pt * cos_var;
    proj_sin = in_pt * sin_var;
