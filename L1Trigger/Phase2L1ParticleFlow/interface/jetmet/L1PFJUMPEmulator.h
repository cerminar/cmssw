...
namespace L1JUMPEmu {
  ...
  // typedef ap_fixed<64, 32> proj_t;
  // typedef ap_fixed<64, 32> proj2_t;
  ...
  inline Particle_xy Get_xy(l1ct::Sum in_particle) {
    ...
    poly2_t cos2_par0[16] = {-1.00007,
                             ...
                             -0.706719,
                             -0.923708};
    poly2_t cos2_par1[16] = {9.164680268990924e-06,
                             ...
                             -0.001689531075808071};
    poly2_t cos2_par2[16] = {9.319674765430664e-06,
    ...
    poly2_t sin2_par0[16] = {0.000524872,
                             -0.382229,
                             -0.706791,
                             -0.923959,
                             -1.00008,
                             ...
                             0.707535,
                             0.3832};
    poly2_t sin2_par1[16] = {-0.004431478237276202,
                             -0.00409041472149773,
                             ...
                             -0.0040978672774037465};
    poly2_t sin2_par2[16] = {1.870674613498914e-06,
    ...
    int phibin = 0;
    if (in_particle.hwPhi < phi2_edges[1])
      phibin = 0;
    else if (in_particle.hwPhi < phi2_edges[2])
      phibin = 1;
    else if (in_particle.hwPhi < phi2_edges[3])
      phibin = 2;
    else if (in_particle.hwPhi < phi2_edges[4])
      phibin = 3;
    else if (in_particle.hwPhi < phi2_edges[5])
      phibin = 4;
    else if (in_particle.hwPhi < phi2_edges[6])
      phibin = 5;
    else if (in_particle.hwPhi < phi2_edges[7])
      phibin = 6;
    else if (in_particle.hwPhi < phi2_edges[8])
      phibin = 7;
    else if (in_particle.hwPhi < phi2_edges[9])
      phibin = 8;
    else if (in_particle.hwPhi < phi2_edges[10])
      phibin = 9;
    else if (in_particle.hwPhi < phi2_edges[11])
      phibin = 10;
    else if (in_particle.hwPhi < phi2_edges[12])
      phibin = 11;
    else if (in_particle.hwPhi < phi2_edges[13])
      phibin = 12;
    else if (in_particle.hwPhi < phi2_edges[14])
      phibin = 13;
    else if (in_particle.hwPhi < phi2_edges[15])
      phibin = 14;
    else if (in_particle.hwPhi >= phi2_edges[15])
      phibin = 15;
    ...
    poly_t cos_var =
        cos2_par0[phibin] + cos2_par1[phibin] * (in_particle.hwPhi - phi2_edges[phibin]) +
        cos2_par2[phibin] * (in_particle.hwPhi - phi2_edges[phibin]) * (in_particle.hwPhi - phi2_edges[phibin]);
    poly_t sin_var =
        sin2_par0[phibin] + sin2_par1[phibin] * (in_particle.hwPhi - phi2_edges[phibin]) +
        sin2_par2[phibin] * (in_particle.hwPhi - phi2_edges[phibin]) * (in_particle.hwPhi - phi2_edges[phibin]);
  ...
  inline void Met_dPt(std::vector<l1ct::Jet> jets, proj2_t& dPx_2, proj2_t& dPy_2) {
    proj2_t each_dPx2 = 0;
    proj2_t each_dPy2 = 0;
...
inline void JUMP_emu(l1ct::Sum inMet, std::vector<l1ct::Jet> jets, l1ct::Sum& outMet) {
  ...
  L1JUMPEmu::proj2_t dPx_2;
  L1JUMPEmu::proj2_t dPy_2;
  L1JUMPEmu::Met_dPt(jets, dPx_2, dPy_2);

  L1JUMPEmu::Particle_xy outMet_xy;
  outMet_xy.hwPx = (inMet_xy.hwPx > 0) ? inMet_xy.hwPx + L1JUMPEmu::proj2_t(sqrt(dPx_2.to_float()))
                                       : inMet_xy.hwPx - L1JUMPEmu::proj2_t(sqrt(dPx_2.to_float()));
  outMet_xy.hwPy = (inMet_xy.hwPy > 0) ? inMet_xy.hwPy + L1JUMPEmu::proj2_t(sqrt(dPy_2.to_float()))
                                       : inMet_xy.hwPy - L1JUMPEmu::proj2_t(sqrt(dPy_2.to_float()));
